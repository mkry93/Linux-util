...
- lock screen support (lock the screeen between auto and manual wakes)
- trigger suspend whenever normal user call except below a threshold (battery dead before the minimal rtc run). or decide an action for low battery
...
**10**.Doesnt suspend when the level is below first threshold when plugged in. (maybe due to delay calc resulting to 0)
